...
<body class="bg-gray-100 p-3 md:p-6 text-sm md:text-base">
    
    <div class="max-w-5xl mx-auto bg-white p-4 md:p-8 rounded-xl shadow-lg border-t-4 border-primary">

        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            <h2 class="text-xl md:text-2xl font-bold text-gray-800">Input Produk</h2>
            
            <div class="flex flex-wrap gap-4 items-center w-full md:w-auto justify-between md:justify-end">
                <a href="kategori" target="_blank"
                    class="text-sm text-primary hover:text-primaryHover font-bold underline">
                    <i class="fas fa-tags mr-1"></i> Kelola Kategori
                </a>
                <div id="loading_indicator" class="hidden text-primary font-bold animate-pulse flex items-center gap-2 text-xs md:text-sm">
                    <i class="fas fa-spinner fa-spin"></i> <span class="hidden md:inline">Sedang menyimpan...</span>
                </div>
            </div>
        </div>
        ...
        <form id="formProduk" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="nama" id="nama_produk"
                        class="w-full border p-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50"
                        required placeholder="Contoh: Baccarat Rouge">
                </div>
                <div>
                    <label class="block text-sm font-bold mb-1 text-gray-700">Brand</label>
                    <div class="relative">
                        <select name="brand_id"
                            class="w-full border p-2 rounded-lg bg-white appearance-none focus:outline-none focus:ring-2 focus:ring-primary/50">
            ...
            <div class="bg-primary/5 p-4 md:p-6 rounded-xl border border-primary/20 mb-8">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                    <div>
                        <label class="block text-sm font-bold mb-1 text-gray-700">Kategori</label>
                        <div class="flex gap-2">
                            <div class="relative w-full">
                                <select name="category_id" id="category_id" onchange="gantiKategori()"
                                    class="w-full border p-2 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-primary/50">
                                    <option value="">-- Pilih --</option>
                                    <?php while ($k = mysqli_fetch_assoc($kat_query)): ?>
                                        <option value="<?= $k['id'] ?>" data-tipe="<?= $k['tipe_hitung'] ?>">
                                            <?= $k['nama_kategori'] ?></option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
                            <button type="button" onclick="bukaModalKategori()"
                                class="bg-primary text-white px-3 py-2 rounded-lg hover:bg-primaryHover transition shadow-sm"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" id="hpp_input" name="hpp_dasar"
                                class="w-full border p-2 pl-10 rounded-lg font-bold text-gray-800 focus:outline-none focus:ring-2 focus:ring-primary/50"
                                placeholder="0" onkeyup="generateVarianAwal()">
                        </div>
                    </div>
                    <div id="box_karton" class="hidden">
                        <label class="block text-sm font-bold mb-1 text-gray-700">Isi per Karton</label>
                        <input type="number" id="isi_karton" name="isi_karton"
                            class="w-full border p-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50"
                            value="100" onkeyup="generateVarianAwal()">
                    </div>
                </div>
            </div>
            ...
            <div class="mb-8">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-3 gap-2">
                    ...
                    <button type="button" onclick="tambahBarisManual()"
                        class="w-full md:w-auto bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-blue-700 shadow transition transform hover:-translate-y-0.5 flex items-center justify-center gap-2">
                        <i class="fas fa-plus"></i> Tambah Ukuran Custom
                    </button>
                </div>

                <div class="overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
                    <table class="w-full text-sm min-w-[600px]"> <thead class="bg-gray-800 text-white">
                            <tr>
                                <th class="p-3 w-1/4 text-left whitespace-nowrap">Ukuran</th>
                                <th class="p-3 w-1/5 text-left whitespace-nowrap">Modal (Total)</th>
                                <th class="p-3 w-1/6 text-left whitespace-nowrap">Margin (%)</th>
                                <th class="p-3 w-1/4 text-left whitespace-nowrap">Harga Jual</th>
                                <th class="p-3 text-center whitespace-nowrap">Hapus</th>
                            </tr>
                        </thead>
                            ...
                            <tr id="empty_row">
                                <td colspan="5" class="p-8 text-center text-gray-400 bg-gray-50">
                                    <div class="flex flex-col items-center gap-2">
                                        ...
                                        <span>Belum ada settingan otomatis.<br>Silakan pilih Kategori & isi HPP di
                                            atas.</span>
                                    </div>
                                </td>
                            </tr>
                ...
                <p class="text-xs text-gray-400 mt-2 md:hidden italic text-center">
                    <i class="fas fa-arrows-alt-h mr-1"></i> Geser tabel ke samping untuk melihat detail
                </p>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-8">
                <div>
                    <label class="block text-sm font-bold mb-1 text-gray-700">Foto Utama (Cover)</label>
                    <input type="file" name="foto" id="foto_input" required
                        class="w-full border p-2 rounded-lg bg-white text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20">
                    <p class="text-xs text-gray-400 mt-1">*Foto ini yang akan muncul di halaman depan.</p>
                </div>

                <div>
                    <label class="block text-sm font-bold mb-1 text-gray-700">Galeri Tambahan (Foto/Video)</label>
                    <input type="file" name="foto_galeri[]" multiple
                        class="w-full border p-2 rounded-lg bg-white text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20">
                    <p class="text-xs text-gray-400 mt-1">
                        *Tahan tombol <b>CTRL</b> (atau tekan lama di HP) untuk memilih banyak file.<br>
                        *Support Foto & Video (MP4).
                    </p>
                </div>
            </div>
            ...
            <div class="mb-8">
                ...
                <textarea name="deskripsi" id="deskripsi"
                    class="w-full border p-2 rounded-lg h-24 focus:outline-none focus:ring-2 focus:ring-primary/50"></textarea>
            </div>


            <div class="mb-8 bg-yellow-50 p-4 rounded-xl border border-yellow-200 flex items-start gap-3 shadow-sm">
                <input type="checkbox" name="simpan_aturan" id="simpan_aturan"
                    class="w-5 h-5 text-primary rounded mt-1 cursor-pointer focus:ring-primary shrink-0">
            ...
            <button type="submit"
                class="w-full bg-primary text-white font-bold py-4 md:py-3 rounded-xl hover:bg-primaryHover transition shadow-lg text-lg transform hover:-translate-y-1 sticky bottom-4 md:static shadow-xl md:shadow-none z-10">
        ...
        <div class="bg-white p-6 rounded-xl shadow-2xl w-full max-w-sm transform transition-all scale-100">
            ...
            <form id="formKategoriAjax">
                ...
                <div class="flex gap-3 justify-end">
                    <button type="button" onclick="tutupModalKategori()"
                        class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg font-bold hover:bg-gray-300 transition">Batal</button>
                    <button type="submit"
                        class="bg-primary text-white px-4 py-2 rounded-lg font-bold hover:bg-primaryHover transition shadow">Simpan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function tambahBarisTabel(ukuran, modal, margin, isManual, pengaliAwal = 0) {
            ...
            <td class="p-2">
                <input type="text" name="varian[${index}][ukuran]" value="${ukuran}" 
                       class="w-full min-w-[100px] border p-1.5 rounded-md font-bold text-gray-700 input-ukuran focus:ring-2 focus:ring-primary/50 focus:outline-none"
            ...
            <td class="p-2">
                <input type="number" name="varian[${index}][modal]" value="${modal}" 
                       class="w-full min-w-[80px] border p-1.5 rounded-md font-bold input-modal ${modalBg} focus:ring-2 focus:ring-primary/50 focus:outline-none" 
                       ${modalReadonly}
                       oninput="updateHargaRow(this)">
            </td>
            <td class="p-2">
                <div class="flex items-center">
                    <input type="number" step="any" value="${margin}" 
            ...
            <td class="p-2">
                <input type="number" name="varian[${index}][harga]" value="${hargaJual}" 
                       class="w-full min-w-[100px] border p-1.5 rounded-md font-bold text-primary bg-white focus:ring-2 focus:ring-primary/50 input-harga text-lg" 
                       oninput="updateMarginFromPrice(this)">
            </td>
        ...
        function updateMarginFromPrice(el) {
            let row = el.closest('tr');
            let modal = parseFloat(row.querySelector('.input-modal').value) || 0;
            let hargaManual = parseFloat(el.value) || 0;

            if (modal > 0 && hargaManual > 0) {
                let marginBaru = ((hargaManual - modal) / modal) * 100;
                row.querySelector('.input-margin').value = marginBaru.toFixed(2);
            }
            saveManualRowsState();
        }
        ...
        document.getElementById('formKategoriAjax').addEventListener('submit', function (e) {
            ...
            // FETCH URL TANPA PHP (Clean URL)
            fetch('proses_kategori_ajax', { method: 'POST', body: formData })
                .then(res => res.json())
                .then(data => {
        ...
        document.getElementById('formProduk').addEventListener('submit', function (e) {
            ...
            // FETCH URL TANPA PHP (Clean URL)
            fetch('proses_produk_baru', { method: 'POST', body: formData })
