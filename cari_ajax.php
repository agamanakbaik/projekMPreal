...
// PERBAIKAN: Filter berdasarkan category_id (ID), bukan nama kategori
if (!empty($kategori)) {
    $sql .= " AND products.category_id = '$kategori'";
}
...
if ($sort == 'termurah') {
    $sql .= " ORDER BY (SELECT MIN(harga_jual) FROM product_variants WHERE product_variants.product_id = products.id) ASC";
} elseif ($sort == 'termahal') {
    $sql .= " ORDER BY (SELECT MAX(harga_jual) FROM product_variants WHERE product_variants.product_id = products.id) DESC";
} else {
...
if (mysqli_num_rows($query) > 0) {
    while ($row = mysqli_fetch_assoc($query)) {
        ...
        // Ambil Nama Kategori (Agar label gambar tampil Nama, bukan ID)
        $cat_id = $row['category_id'] ?? 0;
        $q_cat_name = mysqli_query($conn, "SELECT nama_kategori FROM categories WHERE id = '$cat_id'");
        $d_cat = mysqli_fetch_assoc($q_cat_name);
        $nama_kategori_label = $d_cat['nama_kategori'] ?? 'Umum';
        ?>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm hover:shadow-md transition duration-300 overflow-hidden border border-gray-100 dark:border-gray-700 flex flex-col h-full group">
            <div class="relative w-full aspect-square bg-gray-100 dark:bg-gray-700 overflow-hidden">
                ...
                <span class="absolute top-1 right-1 bg-white/90 dark:bg-gray-900/90 px-1.5 py-0.5 text-[8px] font-bold rounded text-primary-600 dark:text-primary-500 uppercase shadow-sm">
                    <?= $nama_kategori_label ?>
                </span>
            </div>
    ...
    echo '<div class="col-span-2 md:col-span-4 lg:col-span-6 text-center py-10 text-gray-500 dark:text-gray-400"><p class="text-sm font-bold">Produk tidak ditemukan.</p></div>';
}
?>